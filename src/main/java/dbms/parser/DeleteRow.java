...
public class DeleteRow extends Parser{
  public DeleteRow(Datasource datasource) throws Exception {
    super(datasource);
  ...
  protected void execute(Matcher matcher, String query) {
    try {
      ...
      String[] splitCondition = condition.split("[and]*[or]*");
      System.out.println(Arrays.toString(splitCondition));
      for (String string : splitCondition){
        String[] columnNameAndValue = string.split("=");
